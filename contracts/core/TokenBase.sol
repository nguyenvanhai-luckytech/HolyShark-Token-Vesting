...
contract TokenBase is BEP20, AccessControl, ReentrancyGuard {
    ...
    function _calculateLinearReleaseAmount(
        ...
        uint256 roundsPassed = (((block.timestamp.sub(releaseTime)).div(vestingTypeInfo.daysPerRound.mul(1 days))).mul(30)).add(1);

        if (roundsPassed >= vestingTypeInfo.releaseRounds) {
            roundReleasedAmount = info.amount.sub(tgeReleasedAmount);
        } else {
            roundReleasedAmount = (info.amount.sub(tgeReleasedAmount)).mul(roundsPassed).div(vestingTypeInfo.releaseRounds.div(30));
        }
    }
